...
static int pinit(lua_State *L) {
  ...
  s->argn = collectargs(argv, &s->flags);
  if (s->argn < 0) { /* Invalid args? */
    print_usage();
    s->status = 1;
    return 0;
  }

  if ((s->flags & FLAGS_NOENV)) {
    lua_pushboolean(L, 1);
  ...
  createargtable(L, argv, s->argc, s->argn);
  ...
  if ((s->flags & FLAGS_VERSION))
    print_version();

  s->status = runargs(L, argv, s->argn);
  if (s->status != LUA_OK)
    return 0;
  return 0;
}
static int working(lua_State *L){
  struct Smain *s = &smain;
  char **argv = s->argv;
  s->status = handle_script(L, argv + s->argn);
  return s->status;
}
static int destruct(lua_State *L){
  struct Smain *s = &smain;
  char **argv = s->argv;
    if ((s->flags & FLAGS_INTERACTIVE)) {
    print_jit_status(L);
    dotty(L);
  } else if (s->argc == s->argn && !(s->flags & (FLAGS_EXEC | FLAGS_VERSION))) {
    if (lua_stdin_is_tty()) {
      print_version();
      print_jit_status(L);
      dotty(L);
...
void *align_alloc(size_t align, size_t size) {
  return _aligned_malloc(align, size);
}
int main(int argc, char **argv) {
  lua_set_allocator(realloc, free, malloc, align_alloc);
  int status;
  lua_State *L;
  if (!argv[0])
    argv = empty_argv;
  ...
  report(L, status);
  status = lua_cpcall(L, working, NULL);
  report(L, status);
  status = lua_cpcall(L, destruct, NULL);
  report(L, status);
  lua_close(L);
